import {
  ...
  const getIcon = (icon) => {
    switch (icon) {
      ...
      case "Dices":
        return <Dices className="w-full h-auto" strokeWidth={1.5} />;
      case "linkedin":
  ...
  const NavButton = ({
    ...
    return (
      ...
      {({ size }) => {
        ...
        bg-background/20 border border-white/30 border-solid backdrop-blur-[6px] shadow-glass-inset group-hover:pause hover:shadow-glass-sm animate-spin-slow"
              aria-label={label}
              name={label}
              prefetch={false}
              scroll={false}
